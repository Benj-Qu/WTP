...
## Table of contents
* [Overview](#overview)
* [Part 1](#part1): `wSender`
* [Part 2](#part2): `wReceiver`
* [Part 3](#part3): Optimizations

<a name="overview"></a>
## Overview

In this project, we build a simple reliable transport protocol, WTP, **on top of UDP**. Our WTP implementation provides inorder, reliable delivery of UDP datagrams in the presence of events like packet loss, delay, corruption, duplication, and reordering.

There are a variety of ways to ensure a message is reliably delivered from a sender to a receiver. We implement a sender (`wSender`) and a receiver (`wReceiver`) that follows the following WTP specification.
...
We implement reliable transport using a sliding window mechanism. The size of the window (`window-size`) will be specified in the command line. `wSender` must accept cumulative `ACK` packets from `wReceiver`.

After transferring the entire file, we send an `END` message to mark the end of connection.
...
To handle cases where `ACK` packets are lost, we implement a 500 milliseconds retransmission timer to automatically retransmit packets that were never acknowledged.
Whenever the window moves forward (i.e., some ACK(s) are received and some new packets are sent out), we reset the timer. If after 500ms the window still has not advanced, we retransmit all packets in the window because they are all never acknowledged.
...
For this part of the assignment, we make a few modifications to the programs written in the previous two sections. Consider how the programs written in the previous sections would behave for the following case where there is a window of size 3:
...
In order to account for situations like this, we modify your `wReceiver` and `wSender` accordingly (save these different versions of the program in a folder called `WTP-opt`):
...
For a more concrete example, here is how our improved `wSender` and `wReceiver` behave for the case described at the beginning of this section:
